...
import { requireVolunteer, requireViewer } from '@/lib/auth/require-admin';
...
import { parseNewsFormData } from '@/lib/utils/news-parser';
import { executeSafeAction } from '@/lib/utils/action-handler';
import { revalidateMultiTenantNews } from '@/lib/cache/revalidate';
...
// ─── CREATE NEWS ─────────────────────────────────────────────────────────────
export const createNews = executeSafeAction(async (formData: FormData) => {
    const user = await requireVolunteer();
    await requirePermission('news', 'create');
    const adminDb = await createClient();

    const raw = parseNewsFormData(formData);
...
// ─── UPDATE NEWS ─────────────────────────────────────────────────────────────
export const updateNews = executeSafeAction(async (id: string, formData: FormData) => {
    const user = await requireVolunteer();
    ...
    if (parsed.data.category_id) {
        ...
        if (catData && catData.tenant_id) { // Nếu danh mục thuộc về chi nhánh
...
// ─── DELETE NEWS ─────────────────────────────────────────────────────────────
export const deleteNews = executeSafeAction(async (id: string) => {
...
// ─── SUBMIT FOR REVIEW ───────────────────────────────────────────────────────
export const submitForReview = executeSafeAction(async (id: string) => {
    const user = await requireVolunteer();
    await requirePermission('news', 'update');
    await enforceTenantScopeForRecord('news', id);
    const adminDb = await createClient();
...
// ─── APPROVE NEWS ────────────────────────────────────────────────────────────
export const approveNews = executeSafeAction(async (id: string, note?: string) => {
    const user = await requireViewer();
    await requirePermission('news', 'update');
    await enforceTenantScopeForRecord('news', id);
    const adminDb = await createClient();
...
// ─── REJECT NEWS ─────────────────────────────────────────────────────────────
export const rejectNews = executeSafeAction(async (id: string, note: string) => {
    const user = await requireViewer();
    await requirePermission('news', 'update');
    await enforceTenantScopeForRecord('news', id);
    const adminDb = await createClient();
