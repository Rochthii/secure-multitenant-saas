...
// Dynamic API Key Failover loop for Gemini on Next.js Server-side
...
export async function POST(req: NextRequest) {
    try {
        ...
        // 2. Fetch security statistics from database
        ...
        if (targetTenantId) {
            logsQuery = logsQuery.eq('tenant_id', targetTenantId);
        }
        const { data: recentLogs } = await logsQuery;

        // 3. Chuẩn bị prompt cho AI Gemini phân tích sức khỏe bảo mật
...
Hãy lập một "BÁO CÁO ĐÁNH GIÁ SỨC KHỎE AN NINH HỆ THỐNG TUẦN (WEEKLY SOC HEALTH REPORT)" cho chi nhánh/tổ chức: ${tenantName}.
...
1. **TIÊU ĐỀ CHÍNH:** Nêu rõ tên báo cáo, tên chi nhánh, chuẩn ISO 27001 & ISO 27017, và thời gian xuất bản.
2. **MỤC 1: ĐÁNH GIÁ SỨC KHỎE TỔNG QUAN (SOC HEALTH SCORE):**
   ...
   - Nhận định tình hình chung hôm nay.
3. **MỤC 2: PHÂN TÍCH NGUY CƠ RỦI RO CHI TIẾT:**
   ...
   - Dự báo rủi ro an ninh trong 7 ngày tới (đặc biệt là bài toán cô lập dữ liệu đa chi nhánh).
...
Lưu ý: Không cần tạo bảng số liệu bằng markdown trong phần phản hồi của bạn vì chúng tôi đã vẽ bảng trực quan trong PDF. Không tự bịa đặt các số liệu nằm ngoài JSON được cung cấp. Viết bằng tiếng Việt chuyên nghiệp.`;
        ...
        console.log(`[Telegram Report] Starting report generation with ${allKeys.length} Gemini API keys...`);
        const reportContent = await generateReportWithGemini(aiPrompt, allKeys);
        console.log(`[Telegram Report] Report generated successfully (${reportContent.length} chars).`);

        // 5. Lấy cấu hình gửi Telegram
        ...
        // 6. KHỞI TẠO VÀ VẼ PDF REPORT CHUYÊN NGHIỆP QUA jsPDF
        console.log(`[Telegram Report] Initializing PDF document...`);
        const doc = new jsPDF();
        ...
        doc.text(pdfText("BÁO CÁO SỨC KHỎE AN NINH SOC TUẦN"), 25, 22);
        ...
        doc.setFillColor(248, 250, 252); // Light gray slate-50
        ...
        for (let i = 0; i < timeline.length; i++) {
            ...
            const barHeight = (item.count / maxCount) * 18; // Max 18mm
        ...
        // --- PAGE 2+: AI CISO DETAILED NARRATIVE ---
        doc.addPage();
        
        // Header/Footer standard layout for Page 2
        doc.setFont(fontName, "normal");
        doc.setFontSize(8);
        doc.setTextColor(128, 128, 128);
        doc.text(pdfText("BÁO CÁO SOC HEALTH REPORT - PTIT THESIS SAAS"), 15, 10);
        ...
        // 7. Output PDF ArrayBuffer
        console.log(`[Telegram Report] PDF document compiled successfully.`);
        const pdfOutput = doc.output('arraybuffer');
        const dateStr = new Date().toISOString().split('T')[0];
        const fileName = `Bao_Cao_An_Ninh_SOC_${dateStr}.pdf`;

        // 8. Send PDF via Telegram API sendDocument
        ...
        formData.append(
            'caption', 
            `📊 *BÁO CÁO SOC HEALTH REPORT CHUYÊN NGHIỆP (PDF)*\n🏢 *Tổ chức:* ${tenantName}\n🛡️ *Hệ thống:* PTIT Thesis Secure SAAS\n📈 *Đã tích hợp:* Điểm an ninh, Biểu đồ log truy cập 24h & Cảnh báo an ninh thực tế!\n📅 *Thời gian:* ${new Date().toLocaleString('vi-VN')}\n💾 _Tệp đính kèm định dạng PDF trực quan đã được gửi bên dưới!_`
        );
        formData.append('parse_mode', 'Markdown');

        console.log(`[Telegram Report] Sending PDF ${fileName} to Chat ID: ${telegramChatId}...`);
        ...
        return NextResponse.json({
            success: true,
            message: `Báo cáo an ninh cho ${tenantName} đã được xuất bản dạng PDF và gửi thành công qua Telegram!`,
            fileName
        });

    } catch (e: any) {
        console.error('[Telegram Report] Critical error:', e);
