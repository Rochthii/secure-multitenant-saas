...
export async function GET(request: Request) {
    ...
    const basePath = isCompany ? 'du-an' : 'transactions';

    const start = page * ITEMS_PER_SITEMAP;
    const end = start + ITEMS_PER_SITEMAP - 1;

    let query = supabase.from('transaction_projects').select('id, slug, updated_at').in('status', ['ongoing', 'completed']).range(start, end);
    if (tenantId) {
    ...
    const urlsXml = (projects || []).map(item => {
        const itemSlug = item.slug || item.id;
        const fullPath = `/${basePath}/hang-muc-du-an/${itemSlug}`;

        const hreflangLinks = LOCALES.map(locale =>
            `        <xhtml:link rel="alternate" hreflang="${locale}" href="${escapeXml(`${baseUrl}/${locale}${fullPath}`)}"/>`
        ).join('\n');
        const xDefault = `        <xhtml:link rel="alternate" hreflang="x-default" href="${escapeXml(`${baseUrl}/vi${fullPath}`)}"/>`;

        return `    <url>
        <loc>${escapeXml(`${baseUrl}/vi${fullPath}`)}</loc>
