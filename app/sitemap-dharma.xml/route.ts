...
export async function GET(request: Request) {
    ...
    const urlsXml = (dharma || []).map((item: any) => {
        ...
        const hreflangLinks = LOCALES.map(locale =>
            `        <xhtml:link rel="alternate" hreflang="${locale}" href="${escapeXml(`${baseUrl}/${locale}/documents/${itemSlug}`)}"/>`
        ).join('\n');
        const xDefault = `        <xhtml:link rel="alternate" hreflang="x-default" href="${escapeXml(`${baseUrl}/vi/documents/${itemSlug}`)}"/>`;
