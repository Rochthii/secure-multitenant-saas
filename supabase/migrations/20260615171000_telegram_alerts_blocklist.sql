-- ==============================================================================
-- MIGRATION: Tích hợp Telegram Alert cho Block/Unblock IP (SOAR)
-- Ngày: 2026-06-15
-- ==============================================================================
...
BEGIN
  ...
  -- hoặc chặn/gỡ chặn IP, hoặc đây là hành động giả lập tấn công để demo
  IF NEW.action IN (
    ...
    message_text := format(
        ...
        CASE 
          ...
          WHEN COALESCE(NEW.severity, 'HIGH') = 'INFO' THEN '🟢'
          ELSE '🟨'
        END,
