...
export function BusinessSidebar({
    ...
    const accentColor = resolvedColor ?? '#D4A017';
    const displayName = resolvedName ?? 'Quản lý Tổ chức';
    const isCompany = resolvedTenantType !== 'tenant';
    ...
    const buildMenuItemsAdapted = (tenantId: string): MenuItem[] => {
        ...
        const items: MenuItem[] = [
            ...
            {
                id: 'content',
                label: isCompany ? 'Trung tâm Nội dung' : 'Quản lý Nội dung',
                icon: Newspaper,
                resource: 'news',
                children: [
                    { href: `${base}/categories`, label: isCompany ? 'Phân loại Dự án' : 'Danh mục', icon: FolderTree, resource: 'news' },
                    { href: `${base}/news`, label: isCompany ? 'Tin tức & Truyền thông' : 'Tin tức & Bài viết', icon: Newspaper, resource: 'news' },
                    { href: `${base}/events`, label: isCompany ? 'Sự kiện & Lịch biểu' : 'Sự kiện / Lịch hoạt động', icon: Calendar, resource: 'events' },
                    // E-Learning: Sử dụng icon chuyên dụng cho doanh nghiệp
                    ...(hasModule('documents') ? [{ 
                        href: `${base}/documents`, 
                        label: isCompany ? 'Hệ thống E-Learning' : 'Pháp thoại / Tài liệu', 
                        icon: isCompany ? GraduationCap : BookOpen, 
                        resource: 'media' 
                    }] : []),
                    { href: `${base}/media`, label: 'Kho lưu trữ (Media)', icon: ImageIcon, resource: 'media' },
                ],
            },
            // ── Công ty: Giải pháp & Trang chuyên biệt ────────────
            ...(isCompany ? [{
                id: 'mcaaron-pages',
                label: 'Cổng thông tin (Public)',
                icon: Globe,
                resource: 'news',
                children: [
                    { href: `${base}/pages`, label: 'Trang tĩnh (Pages)', icon: FileText, resource: 'news' },
                    { href: `${base}/about`, label: 'Hồ sơ năng lực (About)', icon: Info, resource: 'news' },
                    { href: `${base}/faq`, label: 'Hỗ trợ khách hàng (FAQ)', icon: HelpCircle, resource: 'news' },
                    { href: `${base}/homepage`, label: 'Tùy biến Trang chủ', icon: LayoutDashboard, resource: 'settings' },
                    { href: `${base}/homepage/slides`, label: 'Banner quảng bá', icon: Layout, resource: 'settings' },
            ...
            // ── Giới thiệu & Trang nội dung (chỉ cho CHI NHÁNH) ──────────────────────
            ...(!isCompany ? [{
                id: 'appearance',
                label: 'Trang tĩnh & Giới thiệu',
                icon: FileText,
                resource: 'news',
                children: [
                    { href: `${base}/pages`, label: 'Trang tự do (Pages)', icon: FileText, resource: 'news' },
                    { href: `${base}/about`, label: 'Nội dung trang Giới Thiệu', icon: Info, resource: 'news' },
                    { href: `${base}/faq`, label: 'Hỏi đáp (FAQ)', icon: HelpCircle, resource: 'news' },
                    { href: `${base}/settings/domain`, label: 'Tên miền (Domain)', icon: Building2, resource: 'settings' },
                    { href: `${base}/settings`, label: 'Cài đặt chi nhánh', icon: Settings, resource: 'settings' },
                ],
            } as MenuGroup] : []),
            // ── Tương tác (Ẩn nếu cấu hình Tài chính tập trung và không phải Global Admin) ──
            ...((hasModule('centralized_finance') && !canSwitchTenant) ? [] : [{
                id: 'interaction',
                label: isCompany ? 'Tài chính & Đối tác' : 'Tương tác & Nhân sự',
                icon: Handshake,
                resource: 'transactions',
                children: [
                    { href: `${base}/transactions`, label: isCompany ? 'Ngân sách & Quỹ' : 'Thanh toán nội bộ', icon: DollarSign, resource: 'transactions' },
                    { href: `${base}/projects`, label: isCompany ? 'Dự án & Phân bổ' : 'Hạng mục nội bộ', icon: FolderTree, resource: 'settings' },
                    { href: `${base}/organizations`, label: isCompany ? 'Danh sách Đối tác' : 'Mạng lưới Tổ chức', icon: Building2, resource: 'tenants' },
                    // Lịch đăng ký: ẩn khi modules_config.registrations = false (DB-driven)
                    ...(hasModule('registrations') ? [{ href: `${base}/events/calendar`, label: isCompany ? 'Lịch hẹn / Đăng ký' : 'Lịch đăng ký sự kiện', icon: UserCheck, resource: 'registrations' }] : []),
                    { href: `${base}/messages`, label: isCompany ? 'Trung tâm Phản hồi' : 'Tin nhắn liên hệ', icon: Mail, resource: 'registrations' },
                ],
            } as MenuGroup]),
            // ── Quản lý Nhân sự & Phân quyền (Delegated Administration) ──
            {
                id: 'hr-management',
                label: isCompany ? 'Nhân sự & Phân quyền' : 'Thành viên & Quyền',
                icon: Shield,
                resource: 'users',
                children: [
                    { href: `${base}/users`, label: isCompany ? 'Danh sách Nhân viên' : 'Danh sách Thành viên', icon: Users, resource: 'users' },
                ],
            },
            // ── Thiết kế Giao diện (chỉ cho CHI NHÁNH) ──
            ...(!isCompany ? [{
                id: 'page-builder',
                ...
                resource: 'settings',
                children: [
                    { href: `${base}/homepage`, label: 'Sắp xếp Trang Chủ', icon: LayoutDashboard },
                    { href: `${base}/homepage/slides`, label: 'Hero Slides / Banner', icon: Layout },
                    ...(role === 'super_admin' ? [{ href: `${base}/theme`, label: 'Giao diện (Theme)', icon: Palette }] : []),
                ],
            } as MenuGroup] : []),
            // ── Cài đặt (chỉ cho công ty — vì chi nhánh đã có trong 'appearance') ────
            ...(isCompany ? [{
                id: 'mcaaron-settings',
                ...
                children: [
                    { href: `${base}/settings/domain`, label: 'Tên miền (Domain)', icon: Building2, resource: 'settings' },
                    { href: `${base}/settings`, label: 'Cài đặt tổ chức', icon: Settings, resource: 'settings' },
                ],
            } as MenuGroup] : []),
        ];
        return items;
    ...
    return (
        <aside className="w-64 bg-[#1C1008] text-white min-h-screen flex flex-col shadow-xl z-50">
            <div
