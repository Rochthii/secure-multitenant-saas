...
Lưu ý: Không cần tạo bảng số liệu bằng markdown trong phần phản hồi của bạn vì chúng tôi đã vẽ bảng trực quan trong PDF. Không tự bịa đặt các số liệu nằm ngoài JSON được cung cấp. Viết bằng tiếng Việt chuyên nghiệp.`;
    ...
    if (!reportContent) {
        ...
        if (allKeys.length === 0) {
            throw new Error(`Cả hai công cụ AI đều thất bại. Groq: ${groqErrorMsg}. Gemini: Chưa cấu hình Gemini API Keys (No Gemini API Keys configured).`);
        }
        ...
        try {
            ...
            throw new Error(`Cả hai công cụ AI đều thất bại. Lỗi Groq: ${groqErrorMsg}. Lỗi Gemini: ${geminiErr?.message || String(geminiErr)}`);
        }
    }
