/**
 * Module sinh mã HTML giao diện báo lỗi an ninh đa ngôn ngữ (vi, en, km)
 * Tương thích hoàn toàn với Next.js Edge Runtime (Không sử dụng Node.js specific APIs)
 */
...
export const getLockdownHtml = (
    ...
    const messages = {
        ...
        km: {
            title: status === 'SUSPENDED' ? '🚨 ប្រព័ន្ធត្រូវបាន bối cảnhជាបន្ទាន់ (SOAR LOCKDOWN)' : '🔒 ការចូល sử dụng được giới hạn (INTRANET LOCKDOWN)',
            desc: status === 'SUSPENDED' 
                ? 'ស្ថាប័ននេះត្រូវបានផ្អាកដំណើរការជាបណ្តោះอาសន្នដោយសារតែប្រព័ន្ធ SOAR រកឃើញការវាយប្រហារបណ្តាញ ឬតាមបញ្ជាបន្ទាន់ពី Super Admin ដើម្បីការពារទិន្នន័យ។' 
