...
- Triển khai Dashboard Threat Simulator v5 hỗ trợ giả lập **5 kịch bản tấn công thực tế** kèm PostgreSQL EXPLAIN ANALYZE và terminal giải thích lý do chặn (Why Blocked).
...
- **Threat Simulator v5 (Mới — v1.7.0):** Mở rộng lên **5 kịch bản tấn công** giả lập thực tế. Kịch bản 5 là **Honeypot Decoy Trap**: khi kịch bản được kích hoạt, hệ thống ghi nhận ngay IP thực, ghi audit log `risk_score=100`, gọi RPC `block_ip`, phát cảnh báo giọng nói AI trên SOC Dashboard — toàn bộ diễn ra trong < 500ms và có thể quan sát trực tiếp theo thời gian thực.
