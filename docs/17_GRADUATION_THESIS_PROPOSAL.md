...
6. **Xây dựng trang đo đạc thực nghiệm (Performance Benchmarking):** Đo lường trực quan tốc độ xử lý trên **111,000 bản ghi dữ liệu thật**, chứng minh độ phức tạp tiệm cận $O(1)$ và $O(\log N)$ của giải pháp đề xuất.
...
| **Auth** | Supabase Auth (JWT Custom Claims) | Xác thực danh tính và cung cấp identity $O(1)$ cho RLS |
...
- Thiết kế RLS Policy Engine & thuật toán trích xuất claim tiệm cận $O(1)$ trong RAM và $O(\log N)$ Indexed Scan.
...
- **Đo lường hiệu năng thực tế (Performance Benchmarking):**
  ...
  - Phân tích biểu đồScaling: Chứng minh giải pháp đề xuất duy trì độ trễ phẳng ổn định tiệm cận $O(1)$ ở quy mô 100,000 bản ghi.
...
- **Chương 5 (Performance Benchmarking):** Thiết lập môi trường đo lường hiệu năng thật 100% trên **111,000 bản ghi dữ liệu seed thực tế** trên Supabase Cloud. Vẽ biểu đồ so sánh phân kỳ hiệu năng rõ rệt giữa Custom JWT Claims ($O(1)$ optimized) và RLS JOIN ($O(N)$), khắc phục triệt để lỗi TypeScript trên giao diện.
