...
- **Performance Benchmarking:** Đo đạc thực tế độ trễ của 111,000 bản ghi dữ liệu thật, vẽ biểu đồ phẳng tối ưu $O(1)$.
...
* **Baseline 3 (Optimized RLS Custom Claims):** Đọc trực tiếp `tenant_id` từ JWT Claims trong RAM Session. Phép toán phân tách claims trong bộ nhớ đạt độ phức tạp **$O(1)$**, triệt tiêu hoàn toàn chi phí JOIN bảng để xác định ngữ cảnh. Khi kết hợp với B-Tree Index trên trường `tenant_id`, độ phức tạp tổng thể của truy vấn chỉ còn phụ thuộc vào việc quét chỉ mục **$O(\log N_{\text{tenant}})$** (với $N_{\text{tenant}}$ là số dòng của riêng khách hàng đó, cực kỳ nhỏ so với tổng dữ liệu hệ thống), chứng minh chi phí bảo mật (Security Overhead) tiệm cận $O(1)$ và hoàn toàn không gây ảnh hưởng đến hiệu năng.
...
* `Planning Time` (0.285 ms): Tăng nhẹ do database engine phải biên dịch lại câu truy vấn và phân tích cú pháp JSON để trích xuất `tenant_id` từ RAM Session. Đây là chi phí CPU xử lý cố định $O(1)$.
...
### 💬 Câu hỏi 3: "Row-Level Security (RLS) bắt buộc database phải lọc dữ liệu ở từng dòng. Khi dữ liệu phình to lên hàng triệu dòng, điều này có gây nghẽn cổ chai hiệu năng (Performance Bottleneck) do chi phí RLS không? Có phải là hằng số O(1) tuyệt đối không?"
* **Kịch bản trả lời đanh thép:**
  > *"Thưa thầy cô, nhận định của thầy cô hoàn toàn chính xác về mặt lý thuyết hệ quản trị cơ sở dữ liệu. Không thể có một truy vấn CSDL vật lý nào đạt độ phức tạp $O(1)$ tuyệt đối vì nó còn phụ thuộc vào việc lập kế hoạch truy vấn (Query Planner), duyệt cây chỉ mục (B-Tree traversal), và kiểm tra tính toàn vẹn phiên bản (MVCC).*
  > 
  > *Tuy nhiên, đóng góp thực nghiệm khoa học trong đồ án của em là **triệt tiêu hoàn toàn chi phí xác thực ngữ cảnh (Authorization Overhead) về mức hằng số $O(1)$** bằng cách sử dụng **JWT Custom Claims**. Thay vì tốn chi phí JOIN bảng $O(\log M)$ truyền thống để tìm `tenant_id` của người dùng, Postgres trích xuất trực tiếp `tenant_id` từ biến bộ nhớ RAM Session (`auth.jwt()`) với độ phức tạp $O(1)$.*
  > 
