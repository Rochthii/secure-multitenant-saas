...
### 🚨 Động cơ SOAR & Phòng vệ chủ động Phân tầng (Tiered Active Defense)
- **Hạn chế lỗ hổng Reverse DDoS (Phản DDoS):** Bản SOAR cũ tự động khóa Tenant khi phát hiện bị tấn công dồn dập ($\ge 3$ vi phạm/phút) dẫn đến lỗ hổng Reverse DDoS nghiêm trọng, cho phép hacker dễ dàng đánh sập chi nhánh/nạn nhân bằng vài request lỗi.
- **Phản ứng Phân tầng linh hoạt (Tiered Response):** Hệ thống SOAR mới chuyển dịch sang 3 cấp độ giảm thiểu linh hoạt:
  * **Tầng 1 (Chặn IP tại Edge):** Khi phát hiện IP lạ vi phạm $\ge 3$ lần/phút, SOAR tự động chèn IP đó vào bảng `public.blocked_ips`. Edge Middleware (`middleware.ts`) chặn IP này ngay tại Edge Runtime ($< 4\text{ms}$). Các user hợp pháp khác của Tenant không bị ảnh hưởng.
  * **Tầng 2 (Khóa tài khoản User):** Tự động khóa tài khoản/session User (`is_locked`) nếu hành vi phá hoại được thực hiện bởi một tài khoản đã đăng nhập (Authenticated User).
  * **Tầng 3 (Phong tỏa Tenant):** Chỉ phong tỏa Tenant (`suspended`) khi xảy ra sự cố rò rỉ dữ liệu diện rộng từ tài khoản Admin/Owner hoặc theo lệnh từ Super Admin.
- **Bảo vệ Whitelist của Admin:** SOAR check IP Whitelist để tuyệt đối **không tự khóa nhầm** địa chỉ IP của quản trị viên hợp pháp, loại bỏ nguy cơ Self-lockdown.
- **Telegram Webhook Alert bất đồng bộ:** Sử dụng hàm `net.http_post` để bắn webhook cảnh báo đỏ khẩn cấp kèm Attack Path trực tiếp về Telegram Admin.

### 🧠 Động cơ Phát hiện Bất thường Lai (HBCAD Engine)
Giúp đồ án vượt qua điểm yếu của "Ngưỡng tĩnh thô sơ" bằng cách đánh giá Điểm Rủi ro Tích lũy (CRS) thời gian thực thông qua 3 pha phân tích toán học:
1. **Base Context Risk (ABAC - Điểm Ngữ cảnh):** Trọng số Action Severity $\times$ Temporal Multiplier (phạt x2.5 ngoài giờ hành chính) $\times$ Network Multiplier (phạt x3.5 ngoài IP whitelist).
2. **Behavioral Deviation (Z-Score - Độ lệch chuẩn hành vi):** Đếm tần suất hoạt động trong giờ hiện tại ($x$) đối chiếu với baseline lịch sử trung bình ($\mu$) và độ lệch chuẩn ($\sigma$) của chính User đó:
   $$Z = \frac{x - \mu}{\sigma}$$
3. **Sequential Pattern Penalty (SPP - Điểm phạt chuỗi):** Tự động cộng phạt khi phát hiện chuỗi dò quét RLS (+50 SPP), chuỗi xóa dữ liệu phá hoại (+60 SPP) hoặc chuỗi càn quét SELECT trích xuất thông tin (+40 SPP).
*Kết quả:* Tự động gắn nhãn mức độ nghiêm trọng (`CRITICAL` nếu $CRS \ge 75$, `WARNING` nếu $35 \le CRS < 75$) và cập nhật hiển thị Neon LED trên Cyber SOC Dashboard.
...
### 💬 Câu hỏi 4: "Cơ chế SOAR (Active Defense) tự động khóa tạm thời (Suspend) Tenant hoạt động như thế nào khi bị tấn công? Thiết kế này có lỗ hổng gì và em đã khắc phục ra sao?"
* **Kịch bản trả lời đanh thép:**
  > *"Thưa thầy cô, ban đầu SOAR tự động phong tỏa Tenant sang `suspended` khi phát hiện tấn công dồn dập (3 vi phạm/phút). Tuy nhiên, thiết kế này dính phải lỗ hổng **Reverse DDoS** nghiêm trọng: Kẻ tấn công có thể gửi vài request rác mang `tenant_id` của nạn nhân từ IP lạ ngoài internet để hệ thống tự động khóa Tenant của nạn nhân.*
  > 
  > *Để khắc phục triệt để và đạt chuẩn bảo mật doanh nghiệp, em đã nâng cấp SOAR sang mô hình **Phản ứng Phân tầng (Tiered Mitigation)**:*
